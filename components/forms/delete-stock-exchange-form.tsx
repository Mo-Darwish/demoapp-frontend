...
export function DeleteStockExchangeForm() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await apiClient.delete("/api/v1/sale-order-items/stockexchange_sale_order/", {
        body: JSON.stringify({
      ...
      toast({
        title: "Success",
        description: "Stock exchange order deleted successfully",
      })
      setSaleOrderId("")
      setBrandItemId("")
    } catch (error) {
      toast({
        title: "Error",
        description: "Failed to delete stock exchange order",
        variant: "destructive",
      })
    } finally {
      setLoading(false)
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <div className="space-y-2">
        <Label htmlFor="delete-se-sale-order-id">Sale Order ID</Label>
        <Input
          id="delete-se-sale-order-id"
          type="number"
          value={saleOrderId}
      ...
      <div className="space-y-2">
        <Label htmlFor="delete-se-brand-id">Brand Item ID</Label>
        <Input
          id="delete-se-brand-id"
          type="number"
          value={brandItemId}
      ...
      <Button type="submit" disabled={loading} variant="destructive" className="w-full">
        {loading ? "Deleting..." : "Delete Stock Exchange Order"}
      </Button>
    </form>
  )
}
