...
export function CreateBulkItemsForm() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast({
        title: "✓ Success",
        description: `${items.length} bulk item(s) have been created successfully!`,
      ...
      toast({
        ...
        description:
          error?.message ||
          "Failed to create bulk items. Please check your input and try again.",
    ...
    <form onSubmit={handleSubmit} className="space-y-4">
      {items.map((item, index) => (
        <div key={index} className="space-y-2 p-4 border rounded-lg">
          ...
          <div className="grid grid-cols-3 gap-2">
            <div>
              <Label htmlFor={`sale_order_id_${index}`} className="text-xs">
                Sale Order ID
              </Label>
              <Input
            ...
            <div>
              <Label htmlFor={`brand_item_id_${index}`} className="text-xs">
                Brand Item ID
              </Label>
              <Input
      ...
      <div className="flex gap-2">
        ...
        <Button type="submit" disabled={loading} className="flex-1">
          {loading ? "Creating..." : "Create Bulk Items"}
        </Button>
      </div>
    </form>
  );
